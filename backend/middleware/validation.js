...
const eventValidation = [
  ...
  body("venue.type").isIn(["physical", "online", "hybrid"]).withMessage("Invalid venue type"),
  ...
  body("ticketTypes.*.price").isNumeric({ min: 0 }).withMessage("Ticket price must be a positive number"),
  body("ticketTypes.*.quantity").isInt({ min: 1 }).withMessage("Ticket quantity must be at least 1"),
]
...
const bookingValidation = [
  ...
  body("items.*.ticketTypeId").isMongoId().withMessage("Invalid ticket type ID"),
  body("items.*.quantity").isInt({ min: 1 }).withMessage("Quantity must be at least 1"),
  body("attendees").isArray({ min: 1 }).withMessage("At least one attendee is required"),
  body("attendees.*.firstName").trim().notEmpty().withMessage("Attendee first name is required"),
  body("attendees.*.lastName").trim().notEmpty().withMessage("Attendee last name is required"),
