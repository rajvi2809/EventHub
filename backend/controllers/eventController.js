...
const getEvents = async (req, res) => {
  try {
    ...
    const eventsWithDetails = events.map((event) => ({
      ...event,
      totalTicketsSold: event.ticketTypes.reduce((sum, ticket) => sum + ticket.sold, 0),
      availableTickets: event.ticketTypes.reduce((sum, ticket) => sum + (ticket.quantity - ticket.sold), 0),
      minPrice: Math.min(...event.ticketTypes.map((ticket) => ticket.price)),
      maxPrice: Math.max(...event.ticketTypes.map((ticket) => ticket.price)),
    }))
...
const getEvent = async (req, res) => {
  try {
    const event = await Event.findById(req.params.id).populate("organizer", "firstName lastName avatar bio")
    ...
    const eventWithDetails = {
      ...
      totalTicketsSold: event.ticketTypes.reduce((sum, ticket) => sum + ticket.sold, 0),
      availableTickets: event.ticketTypes.reduce((sum, ticket) => sum + (ticket.quantity - ticket.sold), 0),
      totalRevenue: event.ticketTypes.reduce((sum, ticket) => sum + ticket.price * ticket.sold, 0),
    }
  ...
  try {
    ...
    const analytics = {
      overview: {
        ...
        totalTicketsSold: bookings.reduce((sum, booking) => sum + booking.totalTickets, 0),
      },
